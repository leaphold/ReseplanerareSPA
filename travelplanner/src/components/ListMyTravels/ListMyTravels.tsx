  ...
  React.useEffect(() => {
    if (cookies.user) {
      getAllTravels().then(allTravels => {
        const userTravels = allTravels.filter(travel => travel.user.email === cookies.user.email);
    ...
    <div>
      ...
      <ul>
        {travels.map(travel => (
          <li key={travel.id}>
            <h2>{travel.city}</h2>
            <p>{travel.dateRange.start.toDateString()} - {travel.dateRange.end.toDateString()}</p>
